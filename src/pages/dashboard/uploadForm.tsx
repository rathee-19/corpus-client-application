...
import { ContentType } from "./contentSelector"; // adjust path if necessary
...
const humanize = (exts: string[]) => exts.join(", ");

interface Props {
  mediaType: ContentType; // passed from ContentSelector / dashboard
...
async function uploadChunkBlob(blob: Blob, fileName: string, chunkIndex: number, totalChunks: number, uploadUuid: string) {
...
async function uploadFileInChunks(file: File, uploadUuid: string, onProgress?: (uploadedChunks: number, totalChunks: number) => void, chunkSize = DEFAULT_CHUNK_SIZE) {
...
const UploadForm: React.FC<Props> = ({ mediaType, onDone, onCancel }) => {
  ...
  const validateFile = (file: File, type: string) => {
    ...
    if (type === "document" && (file.type === "application/pdf" || file.type.includes("offic"))) return true;
    if (type === "text" && file.type?.startsWith("text/")) return true;
    return false;
  };
  ...
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validateFile(f, mediaType)) {
      message.error(
        `File "${f.name}" is not allowed for ${mediaType}. Allowed: ${humanize(EXTENSIONS_BY_TYPE[mediaType] || [])}`
      );
  ...
  const onFinish = async (values: any) => {
    // basic validations
    if (!values.title) return message.error("Title is required");
    if (!values.category_id) return message.error("Category is required");
    if (!values.language) return message.error("Language is required");
    if (!values.release_rights) return message.error("Release rights is required");
    ...
    try {
      ...
      const uploadUuid = (crypto && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : `${Date.now()}-${Math.random()}`;
  ...
  return (
    <div style={{ maxWidth: 820, margin: "0 auto", padding: 20 }}>
      <Title level={3}>Upload Content</Title>
      ...
      >
        ...
        <Form.Item name="category_id" label="Category" rules={[{ required: true }]}>
          <Select loading={loadingCats} placeholder="Select category">
            {categories.map(c => (
        ...
        <div style={{ marginBottom: 12, color: '#444' }}>
          <strong>Content type:</strong> {mediaType}
        </div>

        {mediaType === "text" && (
          <Form.Item name="text_content" label="Text content (optional)">
            <TextArea rows={6} placeholder="Paste or type text content (optional) — you can also upload a file below" />
          </Form.Item>
        )}

        <Form.Item label={`File upload ${mediaType !== "text" ? "(required)" : "(optional)"}`}>
          <Space direction="vertical">
            <input
              type="file"
              accept={acceptForType(mediaType)}
              onChange={onFileChange}
              style={{ display: "block" }}
            />
            <div style={{ fontSize: 12, color: "#666" }}>
              Allowed: {humanize(EXTENSIONS_BY_TYPE[mediaType] || [])}
            </div>
            {selectedFile && (
        ...
        <Form.Item name="release_rights" label="Release Rights" rules={[{ required: true }]}>
          <Radio.Group>
            <Radio value="creator">This work is created by me and free to use</Radio>
            <Radio value="others">Others (attribution / restricted)</Radio>
            <Radio value="downloaded">Downloaded / not sure</Radio>
            <Radio value="NA">NA</Radio>
          </Radio.Group>
        </Form.Item>

        <Form.Item name="creator" label="Creator (optional)">
          <Input placeholder="Creator name (optional)" />
        </Form.Item>

        <Form.Item name="user_id" label="User id (optional)">
          <Input placeholder="user id (optional, will default to current user)" />
        </Form.Item>
